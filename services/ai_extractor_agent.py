...
def run_ai_extractor_autofix(url: str, error_message: str, verify_opts: Optional[Dict] = None) -> Dict:
    ...
    system_prompt = (
        ...
        "OPERATING MODE:\n"
        "1. FIRST PRIORITY: Analyze the provided HTML snippet. Look for direct links, <video> tags, 'og:video' meta tags, or JavaScript redirects (e.g., window.location.href).\n"
        "2. FILE-SHARING SITES: If it's a file hoster (like FileMirage), look for 'Download' buttons, form actions, or JS logic that redirects to the file.\n"
        "3. SECOND PRIORITY: If stuck, use 'web_search'.\n\n"
        "CRITICAL — SITE-WIDE EXTRACTOR:\n"
        "- You MUST create a UNIVERSAL extractor for the ENTIRE website, NOT just for the specific URL provided.\n"
        "- The '_VALID_URL' regex MUST match ANY video/content URL on the site, including ALL subdomains.\n"
        "- Example: for 'https://es.xgroovy.com/gifs/12345/some-title/', the regex should be:\n"
        "  r'https?://(?:[a-z]{2}\\.)?xgroovy\\.com/gifs/(?P<id>\\d+)/(?P<display_id>[^/?#]+)'\n"
        "  NOT: r'https?://es\\.xgroovy\\.com/gifs/...' (this only works for ONE subdomain)\n"
        "- Use (?:www\\.)? or (?:[a-z0-9]+\\.)? to match optional subdomains.\n"
        "- The regex MUST still match the provided URL, but also match other URLs from the same site.\n\n"
    ...
    try:
        for attempt in range(4):  # Steps allowed (e.g. search + search + fix)
            ...
            try:
                ...
                if verify_error:
                    # Extract a meaningful error line (look for ERROR: or the last non-empty line)
                    error_lines = verify_error.strip().splitlines()
                    short_error = verify_error
                    for line in reversed(error_lines):
                        if "ERROR:" in line:
                            short_error = line.strip()
                            break
                        if line.strip():
                            short_error = line.strip()
                            break
